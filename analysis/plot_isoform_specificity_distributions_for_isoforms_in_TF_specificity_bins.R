...
gene_specificity_df$bin <- ceiling(gene_specificity_df$tissue_specificity / 0.2)
...
tf_expression_df$ensembl_gene_id <- tf_metadata_df$ensembl_gene_id[match(tf_expression_df$ensembl_transcript_id, tf_metadata_df$ensembl_transcript_id)]
tf_expression_df$bin <- gene_specificity_df$bin[match(tf_expression_df$ensembl_gene_id, gene_specificity_df$ensembl_gene_id)]

tf_expression_df$mean_expr_in_tissues[is.na(tf_expression_df$mean_expr_in_tissues)] <- 0
tf_expression_df_filtered <- tf_expression_df[tf_expression_df$mean_expr_in_tissues > 1, ]
Plot_df <- tf_expression_df_filtered
Plot_df$bin <- factor(as.character(Plot_df$bin), levels = c("0", "1", "2", "3", "4", "5"))


frequency_table_df <- as.data.frame(table(as.character(as.numeric(as.character(Plot_df$bin)) * 0.2), Plot_df$group))
colnames(frequency_table_df) <- c("bin", "frequency")
frequency_table_df$label <- paste("N", 
	frequency_table_df$frequency, sep = "=")
frequency_table_df$expr_yaxis <- max(Plot_df$mean_expr_in_tissues, na.rm = T)
frequency_table_df$specificity_yaxis <- min(Plot_df$tissue_specificity)


# Breaks <- sort(as.numeric(as.character(unique(tf_expression_df$bin))))
# Labels <- (Breaks) * 0.2
# default_theme <- .fetch_barplot_theme()
# the_plot <- ggplot(Plot_df, aes(x = bin, y = tissue_specificity)) +
# geom_violin(aes(fill = group), colour = "#000000", position = position_dodge(width = 0.7)) +
# geom_boxplot(aes(fill = group), colour = "#000000", position = position_dodge(width = 0.7),
# 	width = 0.1, outlier.shape = NA, outlier.size = NA) +
# geom_label(data = frequency_table_df, aes(x = bin, y = specificity_yaxis, label = label)) +
# scale_fill_brewer("group", palette = "Set1") +
# scale_x_discrete("TF specificity bins", breaks = factor(as.character(Breaks)), labels = Labels) +
# scale_y_continuous("Isoform tissue specificity") +
# default_theme + theme(legend.position = "bottom") + 
# labs(title = "mean isoform expression in TF bins")
# ggsave(file = file.path(analysis_dir, paste(output_prefix(), 
# 	"tissue_specificity_distribution_in_different_TF_specificity_bins_binwidth_0.2_buffered_mean_0.80.pdf", sep = "-")), 
# the_plot, height = unit(9, "cm"), width = unit(9, "cm"))


Plot_df <- Plot_df[Plot_df$bin != "0",]
Breaks <- sort(as.numeric(as.character(unique(Plot_df$bin))))
Labels <- Breaks * 0.2
default_theme <- .fetch_barplot_theme()
Plot_df$group <- factor(Plot_df$group, levels = c("DBD+", "DBD-"))
the_plot <- ggplot(Plot_df, aes(x = bin, y = tissue_specificity)) +
stat_summary(fun = "median", aes(fill = group), size = 1.2, geom = "point", shape = 21) +
stat_summary(fun = "median", aes(colour = group, group = group), geom = "line") +
scale_fill_brewer("group", palette = "Set1") +
scale_colour_brewer("group", palette = "Set1") +
scale_x_discrete("TF specificity bins", breaks = factor(as.character(Breaks)), labels = Labels) +
# scale_x_continuous("TF specificity bins", breaks = c(2,4,6,8,10), labels = c(0.2, 0.4, 0.6, 0.8, 1)) +
scale_y_continuous("median Isoform specificity") +
default_theme + theme(legend.position = "bottom") + 
labs(title = "Tissue specificity distribution in domain groups")
ggsave(file = file.path(analysis_dir, paste(output_prefix(), 
	"median_tissue_specificity_profile_in_different_TF_specificity_bins_binwidth_0.2_buffered_mean_0.80.pdf", sep = "-")), 
the_plot, height = unit(9, "cm"), width = unit(9, "cm"))
